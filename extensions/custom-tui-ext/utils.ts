...
export const MIN_TIPS_WIDTH = 18;
export const MAX_TIPS_WIDTH = 36;
const COLUMN_GAP = 3;

/**
 * Calculate left/right column widths for the header layout.
 * On narrow terminals the tips column is hidden; on wide terminals
 * the tips get progressively more space (up to 36 chars).
 */
export function headerColumnWidths(
	...
	// Scale tips width with terminal size: wider terminals get a bigger tips column
	const ratio = innerWidth >= 140 ? 0.35 : innerWidth >= 100 ? 0.30 : 0.25;
	let rightWidth = Math.min(maxTipsWidth, Math.max(minTipsWidth, Math.round(innerWidth * ratio)));
	...
	if (leftWidth <= rightWidth) {
		leftWidth = Math.ceil((innerWidth - gap) * 0.62);
		rightWidth = innerWidth - gap - leftWidth;
	}
